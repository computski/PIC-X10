#include <xc.h>
#include <stdint.h>
eeprom char msg1[]="X10 SUBSYSTEM 2v4 2022-09-23    ";


/*SUBSYSTEM X10
2022-09-11

in this version. we transmit code C all lights on.  frame needs to transmit twice with no gaps, then go idle for at least 6 cycles.
...
#define GATEPWM
//debug, if GATEPWM then PWM is gated with data, else it is continously on for debug
...
//the messages. first element is the message, 2nd is the bit length
/*
#define MSGSIZE 17
//reverse order...2022-09-22 WORKS.  array is scanned from MSGSIZE thro to zero
const volatile uint8_t msg[MSGSIZE+1][2]= {
{0b000000,5},
{0b10,1},
{0b01011001,7},
{0b01011001,7},
...
//2022-09-24 try command all-on, no unit codes sent
//the messages. first element is the message, 2nd is the bit length
#define MSGSIZE 8
//array is scanned from MSGSIZE thro to zero
const volatile uint8_t msg[MSGSIZE+1][2]= {
{0b10,1},
{0b01010110,7},
{0b01011001,7},
...
volatile uint8_t outMsg=255;  //255 denotes stop transmissions
volatile uint8_t outBit;
...
//we want a 4 second count
if (ticks>=40){
	ticks=0;
	//trigger a new X10 transmission, but only if we have achieved sync
	if (sync120KHZ.sync50==1 || sync120KHZ.sync60==1){
		//start transmission
		outMsg = MSGSIZE;
		outBit = msg[outMsg][1];
		
		sGPIO.SPARE1=sync120KHZ.sync50==1?1:0;
		sGPIO.SPARE2=sync120KHZ.sync60==1?1:0;
		//gate ON the 50Hz or 60Hz indicator this stays on for as long as it takes to transmit 2 frames
	}
}

//turn off the 50/60 Hz indicators at end of frame, but only if we have achieved sync
if (outMsg==255){
	if (sync120KHZ.sync50==1 || sync120KHZ.sync60==1){
	sGPIO.SPARE1=0;
	sGPIO.SPARE2=0;
...
#ifdef GATEPWM
//2022-09-07 new block.  Note the PWM duty cycle is always set to zero at end of 1mS pulse
//so the logic here only needs to consider whether an active duty cycle is reqired
if (outMsg!=255){

if ((msg[outMsg][0] & (1<<outBit)) !=0) {
		//go active, set 50% duty cycle to activate the PWM, this is gated by the outgoing databits
		CCPR1L=0x08;
...
//end of transmission state is outBit=255, outMsg=255
...
if (T0IF){
	...
	GPIO=sGPIO.port;
#ifdef GATEPWM
	//set zero percent duty cycle on PWM to inhibit it
	CCPR1L=0x0;
